...
:: Если возвращается 0, запускает webui.py скрипт с командной строкой аргументов.
if %errorlevel% equ 0 (
    REM Был ли запущен батник двойным кликом?
    IF /i "%comspec% /c %~0 " equ "%cmdcmdline:"=%" (
        REM echo Этот скрипт был запущен двойным кликом.
        cmd /k python.exe -m streamlit run webui.py %*
    ) ELSE (
        REM echo Этот скрипт был запущен с помощью командной строки.
        python.exe -m streamlit run webui.py %*
    )
)
